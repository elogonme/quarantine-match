$(document).ready(function () {
    ...
    // Local Storge
    function saveLocalStorage(personOne, personTwo) {
        var newMatch = [personOne, personTwo]
        var existingMatches = localStorage.getItem("matches")
        if (existingMatches === null) {
            var newArray = []
            newArray.push(newMatch)
            localStorage.setItem("matches", JSON.stringify(newArray));
        }

        else {
            var previousMatches = JSON.parse(existingMatches)
    ...
    function fetchMatchApi(person1, person2) {
        ...
        $.ajax(settings).done(function (response) {
            ...
            saveLocalStorage(person1, person2)
